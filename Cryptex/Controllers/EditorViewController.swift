...
let BasicTableViewDragAndDropDataType = NSPasteboard.PasteboardType(rawValue: "BasicTableViewDragAndDropDataType")

class EditorViewController: NSViewController, NSTableViewDataSource, NSTableViewDelegate, CTableViewDelegate, NSTextViewDelegate, WebFrameLoadDelegate {

    weak var documentHandler: cxDocument!

    public var viewDidApearCallback: GeneralCallback?
    ...
    @IBOutlet weak var setPasswordView: SetPasswordView!
    ...
    override func viewDidLoad() {
        ...
        NSWorkspace.shared.notificationCenter.addObserver(self, selector: #selector(self.receiveSleepNote(_:)), name: NSWorkspace.willSleepNotification, object: nil)
        NSWorkspace.shared.notificationCenter.addObserver(self, selector: #selector(self.receiveSleepNote(_:)), name: NSWorkspace.screensDidSleepNotification, object: nil)
        NSWorkspace.shared.notificationCenter.addObserver(self, selector: #selector(self.receiveSleepNote(_:)), name: NSWorkspace.sessionDidResignActiveNotification, object: nil)

        // Callbacks
        self.unlockView.callback = { password in
//            self.documentHandler.
    ...
    override func viewDidAppear() {
        if let callback = self.viewDidApearCallback {
            callback()
        }
    }
...
//            self.lockFileWithFilename(lastFilename)
    ...
    func showLockView(_ filename: String?, callback: UnlockViewCallback? = nil) {
        self.unlockView.filename = filename ?? ""
        self.unlockView.callback = callback
    ...
    func showSetPasswordView(_ filename: String?, callback: SetPasswordViewCallback? = nil) {
        self.setPasswordView.filename = filename ?? ""
        self.setPasswordView.callback = callback
    ...
    func open(usingUrl filename: URL) {
        if decryptedData != nil {
            if self.window!.isDocumentEdited == true {
                ...
                alert.beginSheetModal(for: self.window!, completionHandler: { (returnCode: NSApplication.ModalResponse) in
...
//            self.closeFile()
        ...
        if FileManager.default.isReadableFile(atPath: filename.path) == false {
            self.showMsg("Error reading the file!", info: "Make sure you have access to the file.", alertStyle: NSAlert.Style.critical)
            return
        }
...
//        self.removeLockscreen()
    ...
    func save(data: DecryptedDataType, toFile filename: URL) -> Bool {
        ...
        do {
            ...
            self.showMsg("Error!", info: "JSON serialization error.", alertStyle: NSAlert.Style.critical)
            return false
        }

        guard let zippedData = (jsonData as NSData).gzipped() else {
            self.showMsg("Error!", info: "Could not compress data.", alertStyle: NSAlert.Style.critical)
            return false
        }
        ...
        guard let finalData = PasswordManager.encryptData(zippedData, withPassword:password!, error:&error) else {
            ...
            self.showMsg("Error!", info: "Could not encrypt data: \(description).", alertStyle: NSAlert.Style.critical)
            return false
        }
        ...
        do {
            ...
            self.showMsg("Error writing file!", info: "Make sure you have access to the folder.", alertStyle: NSAlert.Style.critical)
            return false
        }
    ...
    func convertStringToHTML(_ str: String) -> String? {
        ...
        if data == nil {
            self.showMsg("Error", info: "There is some kind of error in data encoding.", alertStyle: NSAlert.Style.critical)
            return nil
        }

        guard let perlFile = Bundle.main.path(forResource: "Markdown", ofType: "pl") else {
            self.showMsg("Error!", info: "Could not find Markdown -> HTML convertor.", alertStyle: NSAlert.Style.critical)
            return nil
        }
...
//    }
    ...
    @IBAction func exportToCX(_ sender: AnyObject?) {
        ...
        passwordFieldLabel.stringValue = "Password:"
        passwordFieldLabel.font = NSFont(name: "Helvetica Neue", size:13.0)
        passwordFieldLabel.alignment = .right
        passwordFieldLabel.isBezeled = false
        passwordFieldLabel.drawsBackground = false
        passwordFieldLabel.isEditable = false
        passwordFieldLabel.isSelectable = false
    ...
    @IBAction func deleteSheet(_ sender: AnyObject?) {
        if textView!.string.count <= 3 || closing == true {
            self._deleteSheet()
            return
        }
    ...
    func tableView(_ tableView: NSTableView, validateDrop info: NSDraggingInfo, proposedRow row: Int, proposedDropOperation dropOperation: NSTableView.DropOperation) -> NSDragOperation {
        return (dropOperation == NSTableView.DropOperation.above ? NSDragOperation.move : NSDragOperation())
    }
